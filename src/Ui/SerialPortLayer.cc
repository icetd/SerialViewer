...
SerialPortLayer::SerialPortLayer()
    : m_select_port(0)
    , m_serialPort(nullptr)
    , m_selected_baudrate(21)  // 默认选择115200
{
}
...
void SerialPortLayer::ShowPortControl()
{
    ...
    // 显示当前选择的波特率数值
    if (!m_baudrate_list.empty() && m_selected_baudrate >= 0 && 
        m_selected_baudrate < static_cast<int>(m_baudrate_list.size())) {
        ImGui::SameLine();
    ...
    if (opened && m_serialPort) {
        ...
        if (!m_baudrate_list.empty() && m_selected_baudrate >= 0 && 
            ...
            ImGui::TextDisabled(u8"波特率: %d  数据位: %d  停止位: %s  校验位: %s", 
                               ...
                               data_bits + 5,
                               stop_bits == 0 ? "1" : stop_bits == 1 ? "1.5" : "2",
                               parity == 0 ? u8"无" : parity == 1 ? u8"奇" : u8"偶");
        }
    } else {
